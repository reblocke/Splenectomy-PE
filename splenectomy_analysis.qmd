...
import stata_setup
stata_setup.config(
    os.environ.get("STATA_HOME", "/Applications/Stata"),
    os.environ.get("STATA_EDITION", "be"),
)
```
...
%%stata
clear
local derived_file : environment SPLENECTOMY_PE_DERIVED
if "`derived_file'" == "" local derived_file "outputs/stata/derived/full_db.dta"
local output_root : environment SPLENECTOMY_PE_OUTPUT
if "`output_root'" == "" local output_root "outputs/stata/quarto"
global SPLENECTOMY_PE_OUTPUT "`output_root'"
capture mkdir "outputs"
capture mkdir "outputs/stata"
capture mkdir "$SPLENECTOMY_PE_OUTPUT"
capture confirm file "`derived_file'"
if _rc {
    di as error "Required derived dataset not found: `derived_file'"
    di as error "Run Splenectomy PE data cleaning.do before rendering this Quarto document."
    exit 601
}
use "`derived_file'"
...
table1_mc, by(splenectomy) ///
		...
		percent_n percsign("%") iqrmiddle(",") sdleft(" (±") sdright(")") missing onecol saving("$SPLENECTOMY_PE_OUTPUT/demographics by splenectomy.xlsx", replace)
```
...
table1_mc, by(splenectomy) ///
		...
		percent_n percsign("%") iqrmiddle(",") sdleft(" (±") sdright(")") missing onecol saving("$SPLENECTOMY_PE_OUTPUT/admit chars by splenectomy.xlsx", replace)
```
...
graph export "$SPLENECTOMY_PE_OUTPUT/PESI by Splenectomy Status.png", as(png) name("Graph") replace
```
...
table1_mc, by(splenectomy) ///
		...
		percent_n percsign("%") iqrmiddle(",") sdleft(" (±") sdright(")") missing onecol saving("$SPLENECTOMY_PE_OUTPUT/rv strain by splenectomy.xlsx", replace)
```
...
ysize(5) ///
scheme(white_tableau)
graph export "$SPLENECTOMY_PE_OUTPUT/PA Diameter by Splenectomy Status.png", as(png) name("Graph") replace
```
...
ysize(5) ///
scheme(white_tableau)
graph export "$SPLENECTOMY_PE_OUTPUT/PA to AA by Splenectomy Status.png", as(png) name("Graph") replace
```
...
table1_mc, by(splenectomy) ///
		...
		percent_n percsign("%") iqrmiddle(",") sdleft(" (±") sdright(")") missing onecol saving("$SPLENECTOMY_PE_OUTPUT/clot chars by splenectomy.xlsx", replace)
```
...
table1_mc, by(splenectomy) ///
		...
		percent_n percsign("%") iqrmiddle(",") sdleft(" (±") sdright(")") missing onecol saving("$SPLENECTOMY_PE_OUTPUT/dvt presence by splenectomy.xlsx", replace)
```
...
graph export "$SPLENECTOMY_PE_OUTPUT/Overlap in Central Assessments.png", as(png) name("Graph") replace
```
...
graph export "$SPLENECTOMY_PE_OUTPUT/Bland Altman Qanadli.png", as(png) name("Graph") replace
```
